 ...
 * - NVR capacity = 2TB per unit, retention calc with 5% motion
 * - Lighting: qty -> notes (Field 12)
 * - Security: ADC/Resideo toggle; generic sensors; panel order Primary first
 * - Structured Wiring: default "Bank", Del at right
 * - Tabs: Control4, Lighting, Cameras, Security, Structured Wiring
 * - Swatch code text under every picker
 */

/* ===== Constants & State ===== */
const CSV_HEADERS=["Quantity","Model","Location","Manufacturer","System","Custom Field 12"];
const SYSTEM_TABS=["Control4","Lighting","Cameras","Security","Structured Wiring"];
...
let state={client:{name:"",phone:"",email:"",address:""},projectName:"",floors:[],selectedFloorId:null,selectedRoomId:null,selectedTab:"Control4",settings:{cameraBitrateMbps:4,sparePercent:20,motionDuty:0.05},racks:[],securityBrand:"ADC"};
const NVR_TB_PER_UNIT=2; // each NVR assumed to be 2TB
...
function renderLighting(r){
  ...
  (r.lighting.banks||[]).forEach((bank,bIdx)=>{
    ...
    (bank.gangs||[]).forEach((g,gIdx)=>{
      ...
      const notes=document.createElement("input"); notes.type="text"; notes.placeholder="Notes (to Field 12)"; notes.value=g.notes||""; notes.oninput=(e)=>g.notes=e.target.value;
      ...
      const c1=document.createElement("div"); c1.className="col"; c1.innerHTML="<label>Type</label>"; c1.appendChild(type);
      const c2=document.createElement("div"); c2.className="col"; c2.innerHTML="<label>Notes (to Field 12)</label>"; c2.appendChild(notes);
    ...
    addGang.onclick=()=>{ bank.gangs.push({type:"Switch",notes:"",colorCode:"WH"}); renderMain(); };
...
function renderCameras(r){
  ...
  // NVRs + Capacity + Per-camera retention with 5% motion
...
function renderSecurity(r){
  ...
  const brandWrap=document.createElement("div"); brandWrap.className="col";
  const label=document.createElement("label"); label.textContent="Platform";
  ...
  brandWrap.appendChild(label); brandWrap.appendChild(brandSel); brandRow.appendChild(brandWrap);
  ...
  // Panels: Primary first, then Remote
  ...
  const panelPresent=document.getElementById("panelPresent"), panelType=document.getElementById("panelType");
...
function renderStructured(r){
  ...
  const add=document.createElement("button"); add.className="btn-sm"; add.textContent="+ Add Bank";
  add.onclick=()=>{ r.networking.banks.push({id:uid(),name:`Bank ${r.networking.banks.length+1}`,jacks:0,colorCode:null}); renderMain(); };
  wrap.appendChild(list); wrap.appendChild(add);
...
function renderSummary(){
  ...
  state.floors.forEach(f=>f.rooms.forEach(r=>{
    ...
    (r.lighting.banks||[]).forEach(b=> (b.gangs||[]).forEach(()=> totals.loads+=1 )); // positions are labels now
...
function exportCSV(){
  ...
  state.floors.forEach(f=>f.rooms.forEach(r=>{
    ...
    // Control4 CORE (per room with video streams)
    ...
    // Keypads: base + color kit + wallplate FP1 (gang not tracked)
    ...
    // Lighting (positions are labels; notes -> Field12)
...
/* ===== Racks (unchanged packer – still used for CSV summary lines) ===== */
